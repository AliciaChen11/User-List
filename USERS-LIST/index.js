...
function renderUserList(data) {
  let dataHTML = ''
  // data.forEach(({avatar, name, surname,id}) => {})
  data.forEach((item) => {
...
function addToFavorite(id) {
  ...
  const user = users.find((user) => user.id === id) // find 一找到符合條件的項目就會停下來回傳該項目，後面的項目都不再看
...
function showUserModal(id) {
  ...
  axios.get(INDEX_URL + id).then((response) => {
    //const { name, surname, birthday, age, gender, region, email } = response.data
